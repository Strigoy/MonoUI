...
frame:SetPoint("BOTTOM", WorldMapTrackQuest, "BOTTOM", 190, 0)
...
frame:SetScript("OnShow", function()
	if config.FogOfWar == true then frame:SetChecked(true) else frame:SetChecked(false) end
end)

-- Update map
hooksecurefunc("WorldMapFrame_Update", function()
	-- If map isn't shown, may as well not process anything
	if not WorldMapFrame:IsShown() or config.FogOfWar == false then return end
